...
namespace SSLCertificateMaker
{
    public partial class MainForm : Form
    {
        ...
        {
            var previouslySelected = cbIssuerSelect.SelectedItem?.ToString();
        ...
        private void BtnMakeCert_Click(object sender, EventArgs e)
        {
            if (btnMakeCert.Text == c_make)
            {
                var args = new MakeCertArgs(
                                            int.Parse(cbKeyStrength.SelectedItem.ToString()),
                                            dateFrom.Value,
                                            dateUntil.Value,
                                            GetCleanDomainArray(),
        ...
        private void MakeCertificate(object Argument)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        var cerFile = issuerFile.EndsWith(".key", StringComparison.OrdinalIgnoreCase) ? issuerFile.Remove(issuerFile.Length - 4) + ".cer" : null;
        ...
        {
            var illegalChars = new HashSet<char>(new char[] { '#', '%', '&', '{', '}', '\\', '<', '>', '*', '?', '/', ' ', '$', '!', '\'', '"', ':', '@' });
        ...
        {
            if (InvokeRequired)
                Invoke((Action<string>)SetStatus, str);
            else
            {
                lblStatus.Text = str;
            }
